	...
	def train(self, X, Y, lr=0.5, epochs=10, batch_size = 100, use_bias = False):
		...
		for epoch in range(epochs):
			while (i+1)*batch_size < X.shape[0]:
				...
				i += 1
			self.forward_pass(X[i*batch_size:])
			self.backward_pass(Y[i*batch_size:], use_bias)
			self.update_weights(lr)
...
plt.plot(np.arange(len(metrics[1]))*10, metrics[1])
plt.xlabel("Epochs")
plt.ylabel("Training Loss")
plt.savefig("Training_Loss.png")
plt.close()

plt.plot(np.arange(len(metrics[2]))*10, metrics[2])
plt.xlabel("Epochs")
plt.ylabel("Training Fscore")
plt.savefig("Training_Fscore.png")
plt.close()
